...
- **Linux 原生发布**：为 Ubuntu 22.04+ / Debian 12+ x64 提供 `.deb` 和 AppImage，窄屏窗口自动回落为单列布局。
...
开始前请在 x64 Ubuntu 22.04+ 或 Debian 12+ 上安装 Linux 版 Claude Science，并准备一个可用的第三方模型 API Key。

1. 从最新 [Release](../../releases/latest) 下载 `.deb` 或 AppImage。
2. Debian/Ubuntu 使用 `sudo apt install ./BioCSSwitch_*.deb`；AppImage 使用 `chmod +x BioCSSwitch_*.AppImage` 后直接运行。
3. 确认 `claude-science` 可从 `PATH` 启动；非标准路径可使用 `SCIENCE_BIN=/absolute/path/to/claude-science BioCSSwitch`。
4. 打开 BioCSSwitch，保持顶部选择「**第三方模型**」，点「**＋ 新建**」，选择来源并填写 API Key。
5. 创建配置后点「**设为当前**」，校验成功后点击「**一键开始**」。

> 你只需要准备自己的第三方 API Key，其余步骤由 BioCSSwitch 自动完成。
>
> AppImage 在部分发行版上需要额外安装 FUSE。更完整的安装与源码构建说明见 [Linux 指南](./docs/LINUX.md)。当前发布仅支持 x64。
